...
namespace OrderProcessing.lib.Activities
{
    public class ExtractDateFromMailMessageActivity : CodeActivity
    {
        static readonly Regex rxDate = new Regex(@"(\d+) (\w{3}) (\d{4}) (\d+): (\d+):(\d+):(\d+)", RegexOptions.Compiled);
        static readonly List<string> months= new List<string>{ "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec" };
        ...
        [Category("Output")]        
        public OutArgument<MailMessage> EmailDate { get; set; }
